<template>
  <main>
    <div class="serach">
      ...
      <div class="system" @click="$router.push('/system/anrpsystem')">
    ...
    <div class="content">
      ...
      <div class="art-cont">
        ...
        <div class="item" v-for="(item,index) in anrpLogs" :key="item + index">
            <span class="content">
            {{item}}
            </span>
        </div>
...
const anrpModel = namespace('deviceAll')
@Component
export default class Anrp extends Vue {
  @anrpModel.State((state:State) => state.deviceAllList) deviceAllList!:DeviceInfo[];
  @anrpModel.State((state:State) => state.anrpLogs) anrpLogs!:string[];
  private state = {
    no: '',
    color: '',
    url: ''
  ...
  private cameraUrl = 'plate_next'

  private anprLogList: string[] = []

  @Watch('deviceAllList', { immediate: true, deep: true })
  updateState () {
    const data = this.deviceAllList.find(el => el.code === 'vehicle_after')
