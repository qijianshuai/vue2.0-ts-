...
@Component
export default class ChanneSystem extends Vue {
  private state = {
    cameraNo: '',
    cameraIp: '',
    cameraPort: '',
  ...
  created () {
    if (this.$route.params.id === 'top') {
      this.selectQuery.deviceCode = 'monitor_entry'
    } else {
      this.selectQuery.deviceCode = 'monitor_exit'
    }
    this.startSelectDevices()
  }
}
